...
vidmode_start(vidmode_state_t *state)
{
	...
	u16 *gamma_r = &state->saved_ramps[0*state->ramp_size];
	u16 *gamma_g = &state->saved_ramps[1*state->ramp_size];
	u16 *gamma_b = &state->saved_ramps[2*state->ramp_size];
			...
			const color_setting_t *setting)
{
	...
	u16 *gamma_r = &gamma_ramps[0*state->ramp_size];
	u16 *gamma_g = &gamma_ramps[1*state->ramp_size];
	u16 *gamma_b = &gamma_ramps[2*state->ramp_size];

	if (state->preserve) {
		/* Initialize gamma ramps from saved state */
		::memcpy_dup(gamma_ramps, state->saved_ramps,
		       3*state->ramp_size*sizeof(u16));
	} else {
		/* Initialize gamma ramps to pure state */
		for (int i = 0; i < state->ramp_size; i++) {
			u16 value = (double)i/state->ramp_size *
				(UINT16_MAX+1);
			gamma_r[i] = value;
			gamma_g[i] = value;
			gamma_b[i] = value;
